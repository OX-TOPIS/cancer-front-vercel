...
const DisplayBloodResults = () => {
  ...
  useEffect(() => {
    const fetchFormula = async () => {
      try {
        ...
        const response = await fetch(`https://cancer-backend-vercel-akt5.vercel.app/patientss/${username}`);
        if (response.ok) {
  ...
  useEffect(() => {
    const fetchFormula = async () => {
      try {
        console.log(`Fetching formula for user: ${username}`);
        const response = await fetch(`https://cancer-backend-vercel-akt5.vercel.app/formulas/${username}`);
        if (response.ok) {
          const data = await response.json();
          console.log('Data received from API:', data);
          if (data.length > 0) {
  ...
  useEffect(() => {
    const fetchCancerTypes = async () => {
      try {
        ...
        const response = await fetch(`https://cancer-backend-vercel-akt5.vercel.app/cancerType/${username}`);
        if (response.ok) {
          const data = await response.json();
  ...
  useEffect(() => {
    const fetchCancerTypes = async () => {
      try {
        console.log(`Fetching cancer types for user: ${username}`);
        const response = await fetch(`https://cancer-backend-vercel-akt5.vercel.app/recordbloodresult/${username}`);
        if (response.ok) {
          const data = await response.json();
            ...
            {recordBlooadResult.map((recordBlooad, index) => (
              ...
              <div className="box-sd mb-2">
                <div className="">
                  ...
                  <img src={"https://cancer-backend-vercel-akt5.vercel.app/" + recordBlooad.picture} className="w-20 h-20" 
                  onClick={() => openPopup("https://cancer-backend-vercel-akt5.vercel.app/" + recordBlooad.picture)}
                  />

...
const PostponeAppointment = () => {
  ...
  useEffect(() => {
    if (username) {
      fetch(`https://cancer-backend-vercel-akt5.vercel.app/PatientAppointment2/${username}/${appointId}`)
        .then(response => {
          if (!response.ok) {
  ...
  useEffect(() => {
    const fetchUserIdLine = async () => {
      ...
      try {
        const response = await axios.get(`https://cancer-backend-vercel-akt5.vercel.app/useridline/${username}`);
  ...
  const handleSubmit = () => {
    fetch(`https://cancer-backend-vercel-akt5.vercel.app/PatientPostpone/${appointId}/${userIdLine}`, {
      method: 'POST',
      headers: {
